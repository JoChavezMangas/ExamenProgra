...
export default function CalendarForm({
  ...
  const EventSchema = Yup.object().shape({
    title: Yup.string().max(255).required('Se requiere añadir un título'),
  ...
  return (
    <FormProvider methods={methods} onSubmit={handleSubmit(onSubmit)}>
      <Stack spacing={3} sx={{ px: 3 }}>
        <RHFTextField name="title" label="Empleado" />

        {/* <RHFTextField name="description" label="Descripción" multiline rows={3} />

        <RHFSwitch name="allDay" label="Todos los días" /> */}
            ...
            <MobileDateTimePicker
              ...
              renderInput={(params) => (
                <TextField
                  ...
                  fullWidth
                  error={!!isDateError}
                  helperText={isDateError && 'Debe ser posterior a la fecha de inicio'}
                />
              )}
            />
          )}
