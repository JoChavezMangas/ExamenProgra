...
export default function BancoNewEditForm({ isEdit = false, currentBanco }) {
  ...
  const NewBancoSchema = Yup.object().shape({
    bankName: Yup
    .string()
    .required('Ingresa nombre del banco'),
    digit: Yup
    .string()
    .required('Ingresa dígitos'),
    idSap: Yup
    .string()
    .required('Ingresa id de banco')
    .matches(/[0-9]|\./, 
  ...
  const onSubmit = async (data) => {
    try {
      ...
      reset();
      enqueueSnackbar(!isEdit ? '¡El banco fue registrado!' : '¡Los cambios fueron actualizados!');
