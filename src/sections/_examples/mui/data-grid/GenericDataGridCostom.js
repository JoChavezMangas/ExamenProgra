...
GenericDataGridCustom.propTypes = {
    ...
    filterColumFunc: PropTypes.any,
    filterValueFunc: PropTypes.any,
};

export default function GenericDataGridCustom({ EndPoint, columns, refresh, genericParamAUX, filterColumFunc = () => {}, filterValueFunc = () => {} }) {
    ...
    const onFilterChange = useCallback((filterModel) => {
        ...
        filterColumFunc(filterModel.items[0].columnField);
        filterValueFunc(filterModel.items[0].value);
    }, [filterColumFunc, filterValueFunc]);
    ...
    function loadServerRows(urlEndpoint, filter_Value, operator_Value, column_Field, paramaAUX) {     
        ...
        return new Promise((resolve) => {
            // console.log("la urlEndpoint", urlEndpoint)
    ...
    const customLabelDisplayedRows = ({ from, to, count }) => `${from}-${to} de ${count}`;
